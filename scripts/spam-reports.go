...
func main() {
	...
	reportersMap, err := ImportExistingAccounts(NUM_OF_REPORTERS)
	if err != nil {
		fmt.Println("Error creating accounts: ", err)
		return
	}
	...
	for {
		...
		// call spam function
		fmt.Println("Calling spam reports for ", querydata)
...
func SpamReportsWithReportersMap(reportersMap map[string]ReporterInfo, qd string) {
	...
	fmt.Println("Num of reporters in map: ", len(reportersMap))
	...
	for reporter_name, info := range reportersMap {
		...
		go func(reporter_info *ReporterInfo, path string) {
			defer wg.Done()
			cmd := exec.Command(COMMAND_PATH, "tx", "oracle", "submit-value", reporter_info.Address, qd, value, "--from", reporter_info.Address, "--chain-id", "layertest-2", "--fees", "15loya", "--keyring-backend", "test", "--keyring-dir", path, "--sequence", strconv.Itoa(reporter_info.SequenceNum), "--home", path, "--node", "http://54.234.103.186:26657", "--yes")
			output, err := cmd.CombinedOutput()
			if err != nil {
...
func StripNonNumbers(input string) string {
...
func GetSequenceNumberForAccount(address string) string {
	...
	numString := resArr[1]
	numString = StripNonNumbers(numString)
	return numString
}

func ImportExistingAccounts(numOfReporters int) (map[string]ReporterInfo, error) {
	reporterMap := make(map[string]ReporterInfo, numOfReporters)
	for i := 1; i <= numOfReporters; i++ {
		address := GetAddressFromKeyName(fmt.Sprintf("test_reporter%d", i))
		sequence := GetSequenceNumberForAccount(address)
		seqNum, err := strconv.Atoi(sequence)
		if err != nil {
			return nil, err
		}
		reporterMap[fmt.Sprintf("test_reporter%d", i)] = ReporterInfo{Address: address, SequenceNum: seqNum}
	}

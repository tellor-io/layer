...
PACKAGE_NAME          := github.com/tellor-io/layer
...
## build: Build the layerd binary into the ./build directory.
build: mod
	@cd ./cmd/layerd
	@mkdir -p build/
...
## install: Build and install the layerd binary into the $GOPATH/bin directory.
install: go.sum
	@echo "--> Installing layerd"
	@go install $(BUILD_FLAGS) ./cmd/layerd
.PHONY: install
...
## build-docker: Build the layerd docker image. Requires docker.
build-docker:
	@echo "--> Building Docker image"
	$(DOCKER) build -t tellor-io/layer -f Dockerfile .
.PHONY: build-docker

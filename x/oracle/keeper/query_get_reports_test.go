...
func (s *KeeperTestSuite) TestGetReportsByQueryId() {
	...
	report2, err := s.oracleKeeper.GetReportsbyReporter(ctx, &types.QueryGetReportsbyReporterRequest{Reporter: Addr.String()})
	require.Equal(&expectedReport, report2)
	report3, err := s.oracleKeeper.GetReportsbyReporterQid(ctx, &types.QueryGetReportsbyReporterQidRequest{Reporter: Addr.String(), Qid: "83a7f3d48786ac2667503a61e8c415438ed2922eb86a2906e4ee66d9a2ce4992"})
	require.Equal(&expectedReport, report3)
}

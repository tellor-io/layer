...
type ReportByBlockIndexes struct {
	BlockNumber *indexes.Multi[uint64, collections.Pair[[]byte, collections.Pair[uint64, []byte]], types.DelegationsAmounts]
}

func (b ReportByBlockIndexes) IndexesList() []collections.Index[collections.Pair[[]byte, collections.Pair[uint64, []byte]], types.DelegationsAmounts] {
	return []collections.Index[collections.Pair[[]byte, collections.Pair[uint64, []byte]], types.DelegationsAmounts]{b.BlockNumber}
}
...
func newReportByBlockIndexes(sb *collections.SchemaBuilder) ReportByBlockIndexes {
	return ReportByBlockIndexes{
		BlockNumber: indexes.NewMulti(
			...
			func(pk collections.Pair[[]byte, collections.Pair[uint64, []byte]], _ types.DelegationsAmounts) (uint64, error) {
				return pk.K2().K1(), nil // blockNumber
			},
		),
	}
}

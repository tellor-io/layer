...
func (s *KeeperTestSuite) TestSlashAndJailReporter() {
	...
	s.reporterKeeper.On("JailReporter", s.ctx, reporterAcc, uint64(0), report.BlockNumber).Return(nil)
...
func (s *KeeperTestSuite) TestJailReporter() {
	...
	s.reporterKeeper.On("JailReporter", s.ctx, reporterAcc, uint64(0), uint64(0)).Return(nil)
	s.NoError(s.disputeKeeper.JailReporter(s.ctx, reporterAcc, uint64(0), uint64(0)))
}

...
func (k *KeeperTestSuite) TestMsgServerAddFeeToDispute() {
	...
	// jail duration is 0
	k.reporterKeeper.On("JailReporter", k.ctx, sdk.MustAccAddressFromBech32(dispute.InitialEvidence.Reporter), uint64(0), dispute.InitialEvidence.BlockNumber).Return(nil)
...
func BenchmarkAddFeeToDispute(b *testing.B) {
	...
	rk.On("JailReporter", ctx,
		sdk.MustAccAddressFromBech32(dispute.InitialEvidence.Reporter),
		uint64(0), dispute.InitialEvidence.BlockNumber).Return(nil).Maybe()

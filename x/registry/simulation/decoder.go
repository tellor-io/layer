...
func NewDecodeStore(cdc codec.Codec) func(kvA, kvB kv.Pair) string {
	return func(kvA, kvB kv.Pair) string {
		...
		case bytes.Equal(kvA.Key[:1], types.LastTotalPowerKey):
			var lastTotalPowerA, lastTotalPowerB sdk.IntProto

			cdc.MustUnmarshal(kvA.Value, &lastTotalPowerA)
			cdc.MustUnmarshal(kvB.Value, &lastTotalPowerB)

			return fmt.Sprintf("%v\n%v", lastTotalPowerA, lastTotalPowerB)

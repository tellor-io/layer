...
func TestTempDir_CreatesDirectory(t *testing.T) {
	...
	if dir != app.DefaultNodeHome {
		...
		require.Contains(t, dir, tempDirPrefix, "directory name should contain 'tellorapp'")
	}
}
...
func TestNewRootCmd_CleansUpTempDirectory(t *testing.T) {
	...
	require.LessOrEqual(t, afterCount, beforeCount+1,
...
func TestNewRootCmd_TempDirectoryIsRemoved(t *testing.T) {
	...
	for _, entry := range entries {
		if entry.IsDir() {
			name := entry.Name()
			if len(name) > len(tempDirPrefix) && name[:len(tempDirPrefix)] == tempDirPrefix {
...
func countTellorappDirs(t *testing.T) int {
	...
	for _, entry := range entries {
		if entry.IsDir() {
			name := entry.Name()
			// Check if name starts with tempDirPrefix
			if len(name) >= len(tempDirPrefix) && name[:len(tempDirPrefix)] == tempDirPrefix {
				count++
			}
		}
...
func TestTempDir_FallbackToDefaultNodeHome(t *testing.T) {
	...
	// Otherwise it should return a path containing tempDirPrefix
	if dir == app.DefaultNodeHome {
		// This is the fallback case - acceptable
		require.Equal(t, app.DefaultNodeHome, dir)
	} else {
		// Normal case - should contain tempDirPrefix
		require.Contains(t, dir, tempDirPrefix)
	}
}

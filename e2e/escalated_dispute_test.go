...
func TestEscalatedDispute(t *testing.T) {
	f, err := interchaintest.CreateLogFile("spinup_test.log")
	...
	user1 := interchaintest.GetAndFundTestUsers(t, ctx, "user1", math.NewInt(100*1e6), layer)[0]
	user1Address := user1.FormattedAddress()
	fmt.Println("User1 address: ", user1Address)
	...
	// wait 4 blocks to be safe (3 sometimes works)
	...
	stdout, _, err := validatorI.Exec(ctx, validatorI.TxCommand("user1", "oracle", "tip", user1Address, ltcusdQData, randomTip.String(), "--keyring-dir", "/var/cosmos-chain/layer-1"), validatorI.Chain.Config().Env)
	require.NoError(err)
	...
	// make sure all is square on aggregate repor\
	...
	require.Equal(microReports.MicroReports[0].Power, expectedPower.QuoRaw(layertypes.PowerReduction.Int64()).String())
	...
	expectedTxInfo, _, err := validatorI.ExecQuery(ctx, "tx", txHash)
	require.NotNil(expectedTxInfo)
	require.NoError(err)
	// TODO: unmarshal tx query
	...
	require.NoError(err)
	txHash, err = validatorI.ExecTx(ctx, "user1", "dispute", "propose-dispute", string(bz), "warning", "500000000000loya", "true", "--keyring-dir", "/var/cosmos-chain/layer-1", "--gas", "1000000", "--fees", "1000000loya")
	require.NoError(err)

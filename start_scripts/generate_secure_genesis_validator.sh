...
export MONIKER="calebmoniker"
export KEY_NAME="caleb"
...
./layerd init $MONIKER --chain-id $CHAIN_ID --home ~/.layer
...
sed -i 's/^chain-id = .*$/chain-id = '\"$CHAIN_ID\"'/g' ~/.layer/config/app.toml
...
./layerd keys add $KEY_NAME --keyring-backend $KEYRING_BACKEND --home ~/.layer

echo "set chain id in genesis file to layer..."
sed -i 's/"chain_id": .*"/"chain_id": '\"$CHAIN_ID\"'/g' ~/.layer/config/genesis.json
...
jq '.app_state.globalfee.params.minimum_gas_prices[0].amount = "0.000025000000000000"' ~/.layer/config/genesis.json > temp.json && mv temp.json ~/.layer/config/genesis.json

echo "Set pruning to custom..."
sed -i '' 's/^pruning = "default"/pruning = "custom"/g' ~/.layer/config/app.toml
sed -i '' 's/^pruning-keep-recent = "0"/pruning-keep-recent = "1209600"/g' ~/.layer/config/app.toml
sed -i '' 's/^pruning-interval = "0"/pruning-interval = "10"/g' ~/.layer/config/app.toml

echo "Turn on snapshot service for node"
sed -i '' 's/^snapshot-interval = 0/snapshot-interval = 2000/g' ~/.layer/config/app.toml
sed -i '' 's/^snapshot-keep-recent = 2/snapshot-keep-recent = 5/g' ~/.layer/config/app.toml

# Get address/account for validator to use in gentx tx
echo "Get address/account for $KEY_NAME to use in gentx tx"
VAL=$(./layerd keys show $KEY_NAME -a --keyring-backend $KEYRING_BACKEND --home ~/.layer)
echo "VAL: $VAL"

# Create genesis account to give validator loyas to stake
echo "Adding genesis account for $KEY_NAME..."
./layerd genesis add-genesis-account $VAL 100000000loya --keyring-backend $KEYRING_BACKEND --home ~/.layer

# Create a tx to stake some loyas for validator
echo "Creating gentx for $KEY_NAME..."
./layerd genesis gentx $KEY_NAME 10000000loya --keyring-backend $KEYRING_BACKEND --home ~/.layer --chain-id $CHAIN_ID

echo "Add team address to genesis..."
./layerd genesis add-team-account tellor18wjwgr0j8pv4ektdaxvzsykpntdylftwz8ml97 --home ~/.layer
...
sed -i 's/^laddr = "tcp:\/\/127.0.0.1:26657"/laddr = "tcp:\/\/0.0.0.0:26657"/g' ~/.layer/config/config.toml
...
sed -i 's/^address = "localhost:9090"/address = "0.0.0.0:9090"/g' ~/.layer/config/app.toml
...
sed -i 's/^enabled = false/enabled = true/g' ~/.layer/config/app.toml
# echo "Set prometheus retention time to 60s"
sed -i 's/^prometheus-retention-time = 0/prometheus-retention-time = 60/g' ~/.layer/config/app.toml

echo "Turn on cometBFT prometheus"
sed -i 's/prometheus = false/prometheus = true/' ~/.layer/config/config.toml

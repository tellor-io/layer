...
export HOME_DIR="$HOME/.layer-chains/devnet/layer"
...
# Build layerd from the tag
echo "Building layerd from tag $TAG..."
go build ./cmd/layerd
...
echo "main..."
jq '.app_state.gov.params.voting_period = "5m"' $HOME_DIR/config/genesis.json > temp.json && mv temp.json $HOME_DIR/config/genesis.json
jq '.app_state.gov.params.max_deposit_period = "1m"' $HOME_DIR/config/genesis.json > temp.json && mv temp.json $HOME_DIR/config/genesis.json
...
jq '.app_state.gov.params.expedited_voting_period = "3m"' $HOME_DIR/config/genesis.json > temp.json && mv temp.json $HOME_DIR/config/genesis.json

echo "$KEY_NAME..."
jq '.app_state.gov.params.voting_period = "5m"' $HOME_DIR/$KEY_NAME/config/genesis.json > temp.json && mv temp.json $HOME_DIR/$KEY_NAME/config/genesis.json
jq '.app_state.gov.params.max_deposit_period = "1m"' $HOME_DIR/$KEY_NAME/config/genesis.json > temp.json && mv temp.json $HOME_DIR/$KEY_NAME/config/genesis.json
...
jq '.app_state.gov.params.expedited_voting_period = "3m"' $HOME_DIR/$KEY_NAME/config/genesis.json > temp.json && mv temp.json $HOME_DIR/$KEY_NAME/config/genesis.json

echo "bill..."
jq '.app_state.gov.params.voting_period = "5m"' $HOME_DIR/bill/config/genesis.json > temp.json && mv temp.json $HOME_DIR/bill/config/genesis.json
jq '.app_state.gov.params.max_deposit_period = "1m"' $HOME_DIR/bill/config/genesis.json > temp.json && mv temp.json $HOME_DIR/bill/config/genesis.json
...
jq '.app_state.gov.params.expedited_voting_period = "3m"' $HOME_DIR/bill/config/genesis.json > temp.json && mv temp.json $HOME_DIR/bill/config/genesis.json
...
sed -i '' 's/timeout_commit = "5s"/timeout_commit = "1s"/' $HOME_DIR/bill/config/config.toml
...
# Tip 
#  ./layerd tx oracle tip  \
#   1000000loya \
#   --from alice \
#   --chain-id layer-local-1 \
#   --keyring-backend test \
...
# Submit
# ./layerd tx oracle submit-value  \

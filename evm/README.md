...
### TokenBridgeTransition-FunctionTests

To run TokenBridgeTransition tests, make sure the hardhat forking network is uncommented in `hardhat.config.js`, and the normal `hardhat: {}` network is commented out.

Then run the test:

```bash
npx hardhat test test/TokenBridgeTransition-FunctionTests.js
```

### TokenBridge-FunctionTests

Note that the withdrawFromLayer test will fail for now without Layer time fast-forwarding. 

```bash
npx hardhat test test/TokenBridge-FunctionTests.js
```
...
Then run the test:
```bash
npx hardhat test test/Bridge-TestsAuto.js
```

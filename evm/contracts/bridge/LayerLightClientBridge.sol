...
contract LayerLightClientBridge {
    ...
    function readMultistoreData(
        MultistoreData memory _multiStore
    ) public pure returns(bytes memory) {
        return abi.encodePacked(
            _multiStore.oracleIAVLStateHash,
            _multiStore.paramsStoreMerkleHash,
            _multiStore.slashingToStakingStoresMerkleHash,
            _multiStore.govToMintStoresMerkleHash,
            _multiStore.authToFeegrantStoresMerkleHash,
            _multiStore.transferToUpgradeStoresMerkleHash
        );
    }

    function readBlockHeaderMerkleParts(
        BlockHeaderMerkleParts memory _merkleParts
    ) public pure returns(BlockHeaderMerkleParts memory) {
        return _merkleParts;
    }

    function readCommonEncodedVotePartData(
        CommonEncodedVotePartData memory _commonEncodedVotePart
    ) public pure returns(CommonEncodedVotePartData memory) {
        console.logBytes(_commonEncodedVotePart.signedDataPrefix);
        console.logBytes(_commonEncodedVotePart.signedDataSuffix);
        return _commonEncodedVotePart;
    }

    function readTMSignatureData(
        TMSignatureData memory _signatureData
    ) public pure returns(TMSignatureData memory) {

...
func (s *E2ETestSuite) TestSetUpValidatorAndReporter() {
	...
	require.NoError(err)
	oracleReporter, err := s.reporterkeeper.Reporters.Get(s.ctx, valAddr)
	require.NoError(err)
...
func (s *E2ETestSuite) TestDisputes2() {
	...
	valsAcctAddrs, valsValAddrs, _ := s.CreateValidators(3)
	...
	require.NoError(err)
	s.MintTokens(delAccAddr, math.NewInt(500*1e6))

...
func (s *E2ETestSuite) TestDisputes() {
	...
	reporterDelegator := Delegator{delegatorAddress: reporterAccountAddress, validator: validatorSet[1], tokenAmount: math.NewInt(100 * 1e6)}
	...
	// create dispute
	report := disputetypes.MicroReport{
	...
	_, err = msgServerDispute.ProposeDispute(s.ctx, &disputetypes.MsgProposeDispute{
		Creator:         validatorSet[0].GetOperator(),
		Report:          &report,

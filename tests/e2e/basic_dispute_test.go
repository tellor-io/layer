...
func (s *E2ETestSuite) TestDisputes() {
	...
	skDelegation, err := s.stakingKeeper.Delegation(s.ctx, reporterAccount, valAccountValAddrs[0])
	require.NoError(err)
	...
	fee, err := s.disputekeeper.GetDisputeFee(s.ctx, report, disputetypes.Major)
	require.NoError(err)

	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.ledger {
				...
				cmd = exec.Command("go", "build", "-tags", "ledger", "-o", "layerd", "../../cmd/layerd")
			} else {
				// Generate the binary without Ledger support
				cmd = exec.Command("go", "build", "-o", "layerd", "../../cmd/layerd")
			}
			err := cmd.Run()
			require.NoError(t, err)

			// Clean up the binary
			defer os.Remove("layerd")

			// Run the binary
			cmd = exec.Command("./layerd", "keys", "add", "test-key-name", "--ledger")

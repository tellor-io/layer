...
- 2026-07-13: removed `x/group` entirely (module wiring and KV store, in the same v6.1.6 upgrade) after identifying group proposal execution as a second `MsgServiceRouter` ante-bypass path alongside ICA
- 2026-07-14: corrected `x/group` store removal so v6.1.6 does not mount or migrate the old store key, allowing upgraded nodes to restart; documented the distinction between committed state removal and orphaned physical data
...
Ante-only enforcement is sound only if no execution path runs messages without the ante chain. The app had two such paths, both executing messages straight through the `MsgServiceRouter`. The first was the ICA host — mainnet's ICA host allowlist was found set to `["*"]` (verified against `mainnet.tellorlayer.com`), meaning ICA could bypass not just these caps but the pre-existing delegator cap and 5% tracker. The second was `x/group`: group proposal execution (`doExecuteMsgs`) dispatches a passed proposal's messages directly through the router — the SDK source itself notes the middlewares are bypassed — so a group policy account could delegate, create a validator, or select a reporter past every cap. Rather than duplicating every ante check into handlers, the v6.1.6 upgrade disables interchain accounts entirely and removes `x/group` from the app (see Issues).
...
- **`x/group` is removed entirely.** Group proposal execution shares the exact ante-bypass mechanism as ICA: the group keeper's `doExecuteMsgs` routes a passed proposal's messages through the `MsgServiceRouter` handlers directly, skipping every ante-enforced limit, and any user can create a group and a policy account they fully control. Unlike ICA there is no enable/disable parameter, and the module had zero on-chain and in-repo usage, so the v6.1.6 upgrade removes the module from the app (its `Msg` service is never registered on the router and its types never enter the `InterfaceRegistry`, so group messages can neither be decoded nor routed) and does not mount the `group` KV store key. No deletion migration is configured: the old store is omitted from committed multistore state at the next commit, while underlying orphaned store data may remain in the database instead of being physically purged. Mounting the key only for deletion would make later restarts try to reopen the removed store and fail. The stale `group` entry persisted in the x/upgrade version-map store is not erased, so a module named `group` must never be re-added (it would be treated as migrating, not initializing). `app/group_disabled_test.go` asserts group messages are unroutable, group types are unregistered, the store is unmounted, and the module is absent from the module manager. If group functionality is ever wanted again, re-adding it must come with these limits duplicated in message handlers or a router-level guard.

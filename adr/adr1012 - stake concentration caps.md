...
- **`x/group` is removed entirely.** Group proposal execution shares the exact ante-bypass mechanism as ICA: the group keeper's `doExecuteMsgs` routes a passed proposal's messages through the `MsgServiceRouter` handlers directly, skipping every ante-enforced limit, and any user can create a group and a policy account they fully control. Unlike ICA there is no enable/disable parameter, and the module had zero on-chain and in-repo usage, so the v6.1.6 upgrade removes the module from the app (its `Msg` service is never registered on the router and its types never enter the `InterfaceRegistry`, so group messages can neither be decoded nor routed) and deletes the `group` KV store data via `StoreUpgrades`. Because `StoreUpgrades.Deleted` only walks mounted stores, the store key remains mounted (with no module behind it) for the v6.1.6 release and is unmounted in the following upgrade; the stale `group` entry persisted in the x/upgrade version-map store is not erased, so a module named `group` must never be re-added (it would be treated as migrating, not initializing). `app/group_disabled_test.go` asserts group messages are unroutable, group types are unregistered, the store is unmounted, and the module is absent from the module manager. If group functionality is ever wanted again, re-adding it must come with these limits duplicated in message handlers or a router-level guard.
